...
public abstract class FilteredPreferenceDialog extends PreferenceDialog implements IWorkbenchPreferenceContainer {
	...
	protected class PreferenceFilteredTree extends FilteredTree {
		...
		protected void addFilter(ViewerFilter filter) {
			...
			if (filterText != null) {
				setFilterText(PreferencesMessages.FilteredTree_FilterMessage);
				textChanged();
			}

			cachedTitle = getShell().getText();
			getShell().setText(NLS.bind(PreferencesMessages.FilteredTree_FilteredDialogTitle, cachedTitle));
		}
	...
	protected TreeViewer createTreeViewer(Composite parent) {
		...
		TreeViewer tree;
		if (!hasAtMostOnePage()) {
			filteredTree = new PreferenceFilteredTree(parent, styleBits, new PatternFilter());
		...
		tree.addFilter(new PatternFilter());
	...
	protected void okPressed() {
		...
		if (workingCopyManager != null) {
			try {
				...
				if (msg == null) {
					msg = PreferencesMessages.FilteredPreferenceDialog_PreferenceSaveFailed;
				}
				StatusManager.getManager().handle(new Status(IStatus.ERROR, ProvUIActivator.PLUGIN_ID, IStatus.OK, PreferencesMessages.PreferencesExportDialog_ErrorDialogTitle + ": " + msg, e.getCause()), StatusManager.SHOW);
			}
		}
	...
	Control getContainerToolBar(Composite composite) {
		...
		Action popupMenuAction = new Action() {
			...
			public ImageDescriptor getImageDescriptor() {
				return ProvUIActivator.getDefault().getImageRegistry().getDescriptor(ProvUIImages.IMG_LCL_VIEW_MENU);
			}
			...
			public void run() {
				...
				manager.add(new Action() {
					...
					public String getText() {
						return PreferencesMessages.FilteredPreferenceDialog_Resize;
					}
				});
				manager.add(new Action() {
					/*
					 ...
					 */
					public String getText() {
						return PreferencesMessages.FilteredPreferenceDialog_Key_Scrolling;
					}
				});
		...
		popupMenuAction.setToolTipText(PreferencesMessages.FilteredPreferenceDialog_FilterToolTip);
	...
	void activeKeyScrolling() {
		if (keyScrollingFilter == null) {
			...
			Display display = ProvUI.getDefaultParentShell().getDisplay();
