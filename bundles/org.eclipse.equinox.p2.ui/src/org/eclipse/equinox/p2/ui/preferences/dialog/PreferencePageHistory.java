	...
	public ToolBar createHistoryControls(ToolBar historyBar, ToolBarManager manager) {
		...
		HistoryNavigationAction backward = new HistoryNavigationAction() {
			...
			public boolean isEnabled() {
				...
				if (enabled) {
					setToolTipText(NLS.bind(PreferencesMessages.NavigationHistoryAction_backward_toolTipName, getHistoryEntry(historyIndex - 1).getLabel()));
				}
				return enabled;
			}
		...
		backward.setText(PreferencesMessages.NavigationHistoryAction_backward_text);
		backward.setActionDefinitionId(IWorkbenchCommandConstants.NAVIGATE_BACKWARD_HISTORY);
		backward.setImageDescriptor(ProvUIActivator.getDefault().getImageRegistry().getDescriptor(ProvUIImages.IMG_TOOL_BACK));
		backward.setDisabledImageDescriptor(ProvUIActivator.getDefault().getImageRegistry().getDescriptor(ProvUIImages.IMG_TOOL_BACK_DISABLED));
		...
		HistoryNavigationAction forward = new HistoryNavigationAction() {
			...
			public boolean isEnabled() {
				...
				if (enabled) {
					setToolTipText(NLS.bind(PreferencesMessages.NavigationHistoryAction_forward_toolTipName, getHistoryEntry(historyIndex + 1).getLabel()));
				}
				return enabled;
			}
		...
		forward.setText(PreferencesMessages.NavigationHistoryAction_forward_text);
		forward.setActionDefinitionId(IWorkbenchCommandConstants.NAVIGATE_FORWARD_HISTORY);
		forward.setImageDescriptor(ProvUIActivator.getDefault().getImageRegistry().getDescriptor(ProvUIImages.IMG_TOOL_FORWARD));
		forward.setDisabledImageDescriptor(ProvUIActivator.getDefault().getImageRegistry().getDescriptor(ProvUIImages.IMG_TOOL_FORWARD_DISABLED));
